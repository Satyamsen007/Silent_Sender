...
const SendMessagesPage = () => {
  ...
  const onSubmit = async (data: z.infer<typeof messageSchemaValidation>) => {
    ...
    try {
      ...
      toast.error(axiosError.response?.data.message || 'Faild to send a message')
    } finally {
  ...
  const suggestMessages = async () => {
    ...
    try {
      ...
      const axiosError = error as AxiosError<ApiResponse>
      toast.error(axiosError.response?.data.message || 'Faild to get a suggest messages')
    } finally {
            ...
            <div className="flex flex-col gap-6">
              {
                isLoadingMessages ? (
                  ...
                  messages.map((message, i) => (
                    ...
                    } className="border-solid border-b-2 border-gray-300 text-center hover:bg-gray-300 duration-200 cursor-pointer hover:rounded-md" key={i}>
