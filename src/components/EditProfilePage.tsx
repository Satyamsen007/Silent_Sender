...
const EditProfilePage = () => {
  ...
  const [changeProfileWindow, setChangeProfileWindow] = useState<boolean>(false);
  ...
  const form = useForm<z.infer<typeof updateUserProfileSchemaValidation>>({
  ...
  const handleAccountDeactivation = async () => {
    ...
    try {
      ...
      toast.error(axiosError.response?.data.message || 'Error deactivating account');
    } finally {
  ...
  const handleSubmit = async (data: { username?: string; newPassword?: string; confirmPassword?: string; }) => {
    setIsUpdating(true);
    try {
      const formData = {
    ...
    const checkUserNameUnique = async () => {
      if (userName) {
        ...
        try {
          ...
          const axiosError = error as AxiosError<ApiResponse>;
          setUserNameMessage(axiosError.response?.data.message ?? 'Error while checking user name');
        } finally {
    ...
    <div className="relative">
      ...
      <div className="max-w-5xl relative mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-8">
          ...
          >
            <div className="relative">
              <div className="w-[150px] h-[150px] rounded-full overflow-hidden flex items-center select-none justify-center border-4 border-purple-600">
                {loadAvatar ? <Skeleton className="w-[150px] h-[150px] rounded-full bg-purple-800" /> : <Image src={newAvatar ? newAvatar : session?.user?.avatar as string} alt="avatar" width={112} height={112} className="w-full h-full object-cover" />}
              </div>
