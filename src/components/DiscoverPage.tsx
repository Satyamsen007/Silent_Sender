...
export default function DiscoverPage() {
  ...
  useEffect(() => {
    const getAllUsers = async () => {
      ...
      try {
        ...
        toast.error(axiosError.response?.data.message || 'Failed to fetch all users')
      } finally {
        setFetchUsers(false)
      }
    }
  ...
  return (
    <div className="min-h-screen py-10 px-6">
      ...
      {
        fetchUsers && (
          <div className="h-[60vh] w-full flex justify-center items-center text-2xl text-gray-600"><Loader2 className="w-12 h-12 animate-spin" /></div>
        )
      }
      {allUsers.length <= 0 && fetchUsers || !session ? (
        <div className="h-[60vh] w-full flex justify-center items-center text-2xl text-gray-600">No users available at the moment.</div>
      ) : (
            ...
            >
              <Card className="bg-white text-black p-8 rounded-xl shadow-xl transform hover:scale-105 transition duration-300">
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-gray-600 text-sm mt-2 text-center">Click below to send an anonymous message.</p>
